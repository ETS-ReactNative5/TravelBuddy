...
export function EditProfile({route,navigation }) {
  ...
  const [LoaderFlag, setLoaderFlag] = useState(null);
  ...
  const updateInterface = async (_userNewData) => {
    console.log("inside update interface",_userNewData)
    setImage(_userNewData.image);
    setName(_userNewData.name);
    setGender(_userNewData.gender);
    setDate(prev=>{
      let _date=new Date(_userNewData.dob.seconds*1000)
      // console.log("myData1",_userNewData.dob)
      // console.log("myData1",_date);
      return _date;
    })
    // setDOB(prev=> {
    //   setDate(new Date(_userNewData.dob))
    //   return new Date(_userNewData.dob)
    // });
    setAddress(_userNewData.address);
    setContact(_userNewData.contact);
    setHobbies(_userNewData.hobbies);

  }

 useEffect(async () => {
    const obj = new Controller();
    const result = await obj.showProfile(_userID)
    // console.log(_userID)
    await updateInterface(result);
    setLoaderFlag(result);
        ...
        <KeyboardAvoidingView behavior='padding' keyboardVerticalOffset={keyboardH}>
          ...
          <ScrollView style={{ backgroundColor: 'white', marginBottom: 70 }}>
            <View style={{ paddingVertical: 15 }}>
              <Text style={{ fontSize: 30, color: '#6c63ff', marginBottom: 50, marginLeft: 10, fontWeight: 'bold' }}>Edit Profile</Text>
              <TouchableOpacity onPress={() => toggleUploadFlag()}>
                ...
                <Text style={{ marginBottom: 18, marginLeft: 16, color: '#6c63ff', fontSize: 15 }}>Change Profile Image</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity style={styles.loginBtn} onPress={() => handleSubmit()}>
                <Text style={{ fontWeight: 'bold', fontSize: 18, color: '#e5e5e5' }}>Update</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
